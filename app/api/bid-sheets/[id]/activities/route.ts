...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const body = await req.json()
  const { activity_id, name, description } = body as {
    activity_id?: string
    name?: string
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: rows } = await admin
    ...
    .eq('bid_sheet_activity_id', activityId)
  ...
  try {
    ...
    await admin.from('bid_sheet_items').delete().eq('bid_sheet_id', bidSheetId).eq('bid_sheet_activity_id', activityId)
    const { error } = await admin.from('bid_sheet_activities').delete().eq('id', activityId).eq('bid_sheet_id', bidSheetId)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
