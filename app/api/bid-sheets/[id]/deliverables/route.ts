  ...
  if (before) {
    ...
    if (sheet?.status === 'converted' && sheet.converted_po_id && sheet.site_id) {
      try {
        ...
        await renameDeliverableForProject(
          admin,
          sheet.site_id,
        ...
        const msg = e instanceof Error ? e.message : 'Failed to sync deliverable rename to project budget'
