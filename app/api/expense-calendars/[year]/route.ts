...
/** GET: Signed URL for a year's expense calendar PDF. */
...
/** DELETE: Remove a year's expense calendar (admin/super_admin only). */
export async function DELETE(_req: Request, { params }: { params: Promise<{ year: string }> }) {
  ...
  const { error } = await admin.from('expense_calendars').delete().eq('id', row.id)
  if (error) {
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
