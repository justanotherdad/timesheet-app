...
/** GET: Stream expense calendar PDF inline (same-origin so CSP frame-src 'self' allows iframe embed). */
export async function GET(_req: Request, { params }: { params: Promise<{ year: string }> }) {
  ...
  const safeName = (row.file_name || `expense-calendar-${year}.pdf`).replace(/[^\w.\-() ]+/g, '_')
