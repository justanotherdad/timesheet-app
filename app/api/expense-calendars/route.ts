...
/** GET: List uploaded expense calendars (all authenticated users). */
...
/** POST: Upload or replace an expense calendar PDF for a year (admin/super_admin only). */
export async function POST(req: Request) {
  ...
  const safeName = (file.name || `expense-calendar-${year}.pdf`).replace(/[^\w.\-() ]+/g, '_')
  const storagePath = `expense-calendars/${year}/${Date.now()}-${safeName}`
