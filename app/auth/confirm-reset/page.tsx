 ...
 *
 * Corporate email (Microsoft 365 Safe Links, Barracuda, etc.) prefetches links
 * before users click. The default Supabase flow sends users to Supabase's
 * verify endpoint—which consumes the one-time token on first request. The
 * scanner consumes it before the user can.
 *
 * This page receives the token in the query string (from a custom email
 * template linking to our app). We delay verification by ~2 seconds so that
 * link scanners (which typically don't run JS or wait) don't trigger it. Real
 * users get verified and redirected to the password form.
 */
...
export default function ConfirmResetPage() {
  ...
  }, [tokenHash, type, supabase, router])
